...
CGame_VENTURE_A::CGame_VENTURE_A(UINT32 nConfirmedROMSize)
{
    ...
    m_nLowestKnownPaletteRomLocation = 0x0300000;
    ...
    m_prgGameImageSet = nullptr;  // VENTURE_A_IMG_UNITS -- this is the array of images present in imgdat once that happens
    nImgUnitAmt = 0; // ARRAYSIZE(VENTURE_A_IMG_UNITS); // This is the size of the array tracking which IDs to load from the game's image section
...
int CGame_VENTURE_A::GetExtraCt(UINT16 nUnitId, BOOL bCountVisibleOnly)
{
    ...
    {
        int nDefCtr = 0;
        memset(rgExtraCountAll, 0, ((VENTURE_A_NUMUNIT + 1) * sizeof(int)));

        stExtraDef* pCurrDef = GetExtraDefForVENTURE(0);
        ...
        while (pCurrDef->uUnitN != INVALID_UNIT_VALUE)
        {
            ...
            nDefCtr++;
            pCurrDef = GetExtraDefForVENTURE(nDefCtr);
        }
    }
...
int CGame_VENTURE_A::GetExtraLoc(UINT16 nUnitId)
{
    if (rgExtraLoc[0] == -1)
    {
        ...
        memset(rgExtraLoc, 0, (VENTURE_A_NUMUNIT + 1) * sizeof(int));
...
sDescTreeNode* CGame_VENTURE_A::InitDescTree()
{
    ...
    CString strMsg;
    bool fHaveExtras = (GetExtraCt(VENTURE_A_EXTRALOC) > 0);
    strMsg.Format(L"CGame_VENTURE_A::InitDescTree: Building desc tree for VENTURE_A %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < VENTURE_A_EXTRALOC)
        {
            //Set each description
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", VENTURE_A_UNITS[iUnitCtr].szDesc);
        ...
        if (bUseExtra)
        {
            ...
            CollectionNode = &((sDescTreeNode*)UnitNode->ChildNodes)[(VENTURE_A_EXTRALOC > iUnitCtr) ? (nUnitChildCount - 1) : 0]; //Extra node
            ...
            for (UINT16 nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
            {
                ...
                stExtraDef* pCurrDef = GetExtraDefForVENTURE(nExtraPos + nCurrExtra);

                while (pCurrDef->isInvisible)
                {
                    nCurrExtra++;

                    pCurrDef = GetExtraDefForVENTURE(nExtraPos + nCurrExtra);
                }

                _snwprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, pCurrDef->szDesc);

                ChildNode->uUnitId = iUnitCtr;
                ChildNode->uPalId = (((VENTURE_A_EXTRALOC > iUnitCtr) ? 1 : 0) * nUnitChildCount * 2) + nCurrExtra;
    ...
    strMsg.Format(L"CGame_VENTURE_A::InitDescTree: Loaded %u palettes for VENTURE\n", nTotalPaletteCount);
    OutputDebugString(strMsg);

    m_nTotalPaletteCountForVENTURE = nTotalPaletteCount;
...
sFileRule CGame_VENTURE_A::GetRule(UINT16 nUnitId)
{
    ...
    _snwprintf_s(NewFileRule.szFileName, ARRAYSIZE(NewFileRule.szFileName), _TRUNCATE, L"%u"); // Update with the primary expected ROM name here.
...
UINT16 CGame_VENTURE_A::GetCollectionCountForUnit(UINT16 nUnitId)
{
    ...
    {
        return VENTURE_A_UNITS[nUnitId].uChildAmt;
    }
}
...
UINT16 CGame_VENTURE_A::GetNodeCountForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollectionNode = (const sDescTreeNode*)(VENTURE_A_UNITS[nUnitId].ChildNodes);
...
LPCWSTR CGame_VENTURE_A::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollection = (const sDescTreeNode*)VENTURE_A_UNITS[nUnitId].ChildNodes;
...
UINT16 CGame_VENTURE_A::GetPaletteCountForUnit(UINT16 nUnitId)
{
    ...
    else
    {
        ...
        const sDescTreeNode* pCompleteROMTree = VENTURE_A_UNITS;
...
const sGame_PaletteDataset* CGame_VENTURE_A::GetPaletteSet(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    const sDescTreeNode* pCurrentSet = (const sDescTreeNode*)VENTURE_A_UNITS[nUnitId].ChildNodes;
...
const sDescTreeNode* CGame_VENTURE_A::GetNodeFromPaletteId(UINT16 nUnitId, UINT16 nPaletteId, bool fReturnBasicNodesOnly)
{
    ...
    for (UINT16 nCollectionIndex = 0; nCollectionIndex < nTotalCollections; nCollectionIndex++)
    {
        ...
        {
            const sDescTreeNode* pCollectionNodeToCheck = (const sDescTreeNode*)(VENTURE_A_UNITS[nUnitId].ChildNodes);
...
BOOL CGame_VENTURE_A::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    if (NodeGet->uUnitId != VENTURE_A_EXTRALOC)
    {
